...
<script>
    layui.use(['form', 'table', 'laydate', 'jquery'], function () {
        ...
        //发送ajax请求查询部门下拉列表
        $.get("${pageContext.request.contextPath}/admin/dept/deptList", function (result) {
            var html = "";
            //循环遍历
        ...
        * */
        ...
        function openUpdateWindow(data) {
            mainIndex = layer.open({
                ...
                success: function () {
                    ...
                    //修改的提交地址
                    url = "${pageContext.request.contextPath}/admin/employee/updateEmployee";
                }
            });
        }
        ...
        function openAddWindow() {
            mainIndex = layer.open({
                ...
                success: function () {
                    ...
                    $("#dataFrm")[0].reset();
                    //添加的提交请求
                    url = "${pageContext.request.contextPath}/admin/employee/addEmployee";
                }
            });
        }
        ...
        function deleteById(data) {
            ...
            layer.confirm("确定要删除[<font color='red'>" + data.name + "</font>]吗", {
                ...
                //发送ajax请求进行删除
                $.post("${pageContext.request.contextPath}/admin/employee/deleteById", {"id": data.id}, function (result) {
                    if (result.success) {
                        //刷新数据表格
                        tableIns.reload();
                    }
        ...
        function resetPwd(data) {
            //提示用户是否确认重置
            layer.confirm("确定要重置[<font color='red'>" + data.name + "</font>]的密码吗", {
                icon: 3,
                title: '提示'
            }, function (index) {
                //发送ajax请求进行重置密码
                $.post("${pageContext.request.contextPath}/admin/employee/resetPwd", {"id": data.id}, function (result) {
                    if (result.success) {
                        //刷新数据表格
                        tableIns.reload();
                    }
            ...
            mainIndex = layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    if (checkStatus.data.length > 0) {
                        ...
                        //发送请求
                        $.post("${pageContext.request.contextPath}/admin/employee/saveEmployeeRole", {"roleIds": ids, "empId": data.id}, function (result) {
                            layer.msg(result.message);
                        }, "json");
                        //关闭当前窗口
                        layer.close(mainIndex);
