    ...
    layui.extend({
        ...
        * */
        ...
        function deleteById(data){
            //判断当前角色下是否存在员工
            $.get("${pageContext.request.contextPath}/admin/role/checkRoleHashEmployee",{id:data.id},function (result){
                if(result.exist){
                    //提示用户是否删除该角色
                ...
                yes:function (index,layero){
                    ...
                    if(params.length>0){
                        ...
                        for (var i = 0; i < params.length; i++) {
                            idArr.push(params[i].nodeId);//nodeId是选中的树结点的值,固定不能修改
                        }
                        //将数组转换成字符串
                        var ids=idArr.join(",");
                        //发送ajax请求,保存选中的菜单ID
                        $.post("${pageContext.request.contextPath}/admin/role/saveRoleMenu",{"ids":ids,"roleId":data.id},function (result){
                            layer.msg(result.message);
                        },"json");
                    }else{
                        layer.msg("请选择要分配的菜单");
